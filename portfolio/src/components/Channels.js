...
function Channels({setChannel, sidebarStatus, setSideBarStatus}){
    ...
    useEffect(()=>{
        ...
        function expandSideBar(){
            ...
            const BarinitialClass = "fixed flex xs:top-0 xs:w-screen xs:h-0 lg:left-0 lg:w-0 lg:h-screen flex-col bg-gray-800 h-screen  transition-all 1s ease-in-out opacity-0 z-0";
            const HeaderInitialClass =  " flex fixed xs:w-screen lg:w-80 lg:h-0 bg-gray-800 border-b border-gray-800 shadow-md items-center justify-center transition-all 1s opacity-0"
